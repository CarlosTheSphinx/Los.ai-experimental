...
import { ArrowLeft, Loader2, Paperclip, Upload, X, Archive } from "lucide-react";

const STATUS_LABELS: Record<string, string> = {
  open: "Open", in_progress: "In progress", waiting_on_broker: "Waiting on you", resolved: "Resolved", closed: "Closed",
};
...
function AttachmentChip({ a }: { a: any }) {
  ...
  return (
    <div className="rounded border bg-muted/30 p-2 flex items-center gap-2 text-sm" data-testid={`attachment-${a.id}`}>
      {isImage ? <img src={url} alt={a.fileName} className="h-12 w-12 object-cover rounded" /> : <Paperclip className="h-4 w-4 text-muted-foreground" />}
      <a href={`${url}?download=true`} className="hover:underline truncate flex-1" data-testid={`link-attachment-${a.id}`}>{a.fileName}</a>
...
export default function SupportTicketDetailPage() {
  ...
  const sendReply = useMutation({
    mutationFn: async () => {
      const res = await apiRequest("POST", `/api/support/tickets/${id}/messages`, { body: reply, attachmentObjectPaths: files });
      return res.json();
    },
    onSuccess: () => {
      setReply(""); setFiles([]);
  ...
  const archive = useMutation({
    mutationFn: async () => apiRequest("PATCH", `/api/support/tickets/${id}`, { archivedByBroker: true }),
    onSuccess: () => {
  ...
  const t = data.ticket;
  const ticketAttachments = (data.attachments || []).filter((a: any) => a.ticketId === t.id);
  const messageAttachmentsByMessage: Record<number, any[]> = {};
  for (const a of (data.attachments || [])) {
    if (a.messageId) (messageAttachmentsByMessage[a.messageId] = messageAttachmentsByMessage[a.messageId] || []).push(a);
  }
  ...
  return (
    <div className="container max-w-4xl mx-auto py-8 px-4 space-y-6">
      <div>
        <Link href="/support">
          <Button variant="ghost" size="sm" data-testid="btn-back"><ArrowLeft className="h-4 w-4 mr-1" /> Back to Get Help</Button>
        </Link>
      </div>
      <Card>
        <CardHeader>
        ...
        <CardContent className="space-y-3">
          ...
          {ticketAttachments.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-2">
              {ticketAttachments.map((a: any) => <AttachmentChip key={a.id} a={a} />)}
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <div className="space-y-3">
        {(data.messages || []).map((m: any) => (
          <Card key={m.id} className={m.authorRole === "admin" ? "border-primary/30" : ""} data-testid={`message-${m.id}`}>
            <CardContent className="py-4">
              <div className="flex items-center justify-between mb-2">
                <div className="text-sm font-medium">
                  {m.authorRole === "admin" ? "Lendry Support" : (m.author?.fullName || "You")}
                </div>
                <div className="text-xs text-muted-foreground">{new Date(m.createdAt).toLocaleString()}</div>
              </div>
              <div className="whitespace-pre-wrap text-sm">{m.body}</div>
              {messageAttachmentsByMessage[m.id]?.length > 0 && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-3">
                  {messageAttachmentsByMessage[m.id].map((a: any) => <AttachmentChip key={a.id} a={a} />)}
                </div>
              )}
            </CardContent>
          </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <Textarea value={reply} onChange={e => setReply(e.target.value)} rows={4} placeholder="Add more detail or follow up..." data-testid="input-reply" />
          <input ref={fileRef} type="file" multiple className="hidden" onChange={onPickFiles} />
          <div className="flex items-center gap-2 flex-wrap">
            <Button type="button" variant="outline" size="sm" onClick={() => fileRef.current?.click()} disabled={uploading} data-testid="btn-attach-reply">
