...
-   **Post-Quote Document Selection**: After saving a quote from the pricing results page, users are navigated to `/quotes/:id/documents` (page at `client/src/pages/quote-documents.tsx`) where they can select a document template (Summary PDF or LOI) to download or send for e-signature via PandaDoc. The pricing results page no longer has a Download PDF button or Customer Details form — points/revenue sections display automatically. Features inline PDF preview using `react-pdf` (pages rendered at container width, scrollable), template sidebar for switching between Summary and LOI templates, and an Edit Quote panel (`PATCH /api/quotes/:id`) for modifying borrower/property/loan fields with live PDF regeneration on save.
