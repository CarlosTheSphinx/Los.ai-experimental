import { db } from "../db";
import { funds, intakeDeals, intakeAiAnalysis, intakeDealStatusHistory, intakeDealDocuments } from "@shared/schema";
...
async function callOpenAI(systemPrompt: string, userMessage: string): Promise<any> {
  if (!OPENAI_API_KEY) {
    console.warn("[Intake AI] No OpenAI API key, returning mock analysis");
    return null;
  }
...
async function agent1ValidateAndStructure(deal: any, documents: any[]): Promise<any> {
  const systemPrompt = `You are a Commercial Real Estate Deal Validator AI Agent. Your job is to:
...
Return a JSON object with:
{
  ...
  const userMessage = JSON.stringify({
    deal_data: {
      ...
      property_zip: deal.propertyZip,
      property_value: deal.propertyValue,
      noi_annual: deal.noiAnnual,
      occupancy_pct: deal.occupancyPct,
      borrower_name: deal.borrowerName,
      borrower_entity_type: deal.borrowerEntityType,
      borrower_credit_score: deal.borrowerCreditScore,
      has_guarantor: deal.hasGuarantor,
    },
    documents: documents.filter(d => d.isCurrent).map(d => ({
      type: d.documentType,
...
async function agent2MatchFunds(structuredDeal: any, activeFunds: any[]): Promise<any> {
  const systemPrompt = `You are a Commercial Real Estate Fund Matcher & Risk Analyzer AI Agent. Your job is to:
...
Return a JSON object with:
{
  ...
  "deal_health": {
    "borrower_risk_score": 0-100,
    "borrower_risk_detail": "...",
    "property_risk_score": 0-100,
    "property_risk_detail": "...",
    "loan_structure_risk_score": 0-100,
    "loan_structure_risk_detail": "...",
    "documentation_risk_score": 0-100,
    "documentation_risk_detail": "..."
  }
}`;
  ...
  const userMessage = JSON.stringify({
    ...
    funds: activeFunds.map(f => ({
      ...
      ltv_min: f.ltvMin, ltv_max: f.ltvMax,
      ltc_min: f.ltcMin, ltc_max: f.ltcMax,
      loan_amount_min: f.loanAmountMin, loan_amount_max: f.loanAmountMax,
      allowed_states: f.allowedStates,
      allowed_asset_types: f.allowedAssetTypes,
    })),
  });
...
async function agent3GenerateFeedback(matchingReport: any, structuredDeal: any): Promise<any> {
  const systemPrompt = `You are a Commercial Real Estate Deal Feedback Generator AI Agent. Your job is to:
...
Return a JSON object with:
{
  ...
  if (!result) {
    ...
    return {
      ...
      key_flaws: [],
      strengths: [{ strength: "Deal submitted with required information", detail: "All key fields provided." }],
...
export async function runIntakeAiPipeline(dealId: number): Promise<void> {
  ...
  try {
    ...
    const agent1Result = await agent1ValidateAndStructure(deal, documents);
    ...
    if (agent1Result.validation_status === "invalid" && agent1Result.validation_errors?.length > 0) {
      ...
      await db.insert(intakeDealStatusHistory).values({
        ...
        toStatus: "analyzed",
        notes: "AI analysis complete — validation failed",
      });
      return;
    }

    console.log(`[Intake AI] Agent 2: Matching funds (${activeFunds.length} funds)...`);
    const agent2Result = await agent2MatchFunds(agent1Result, activeFunds);
    ...
    const agent3Result = await agent3GenerateFeedback(agent2Result, agent1Result);
    console.log(`[Intake AI] Agent 3 complete: verdict=${agent3Result.overall_verdict}, confidence=${agent3Result.confidence_score}`);

    await db.insert(intakeAiAnalysis).values({
      dealId,
      agent1Validation: agent1Result,
      agent2Matching: agent2Result,
      agent3Feedback: agent3Result,
      overallVerdict: agent3Result.overall_verdict,
      confidenceScore: agent3Result.confidence_score,
    });

    const newStatus = agent3Result.overall_verdict === "fail" && (agent2Result.eligible_funds?.length || 0) === 0
      ? "no_match"
      : "analyzed";
    ...
    await db.insert(intakeDealStatusHistory).values({
      ...
      fromStatus: "submitted",
      toStatus: newStatus,
      notes: `AI analysis complete — ${agent3Result.overall_verdict} (confidence: ${agent3Result.confidence_score}%)`,
    });
    ...
    await db.insert(intakeAiAnalysis).values({
      dealId,
      agent1Validation: fallback.agent1,
      agent2Matching: fallback.agent2,
      agent3Feedback: fallback.agent3,
      overallVerdict: fallback.agent3.overall_verdict,
      confidenceScore: fallback.agent3.confidence_score,
    });

    const newStatus = fallback.agent3.overall_verdict === "fail" && fallback.agent2.eligible_funds.length === 0
      ? "no_match"
      : "analyzed";
    ...
    await db.insert(intakeDealStatusHistory).values({
      ...
      fromStatus: "submitted",
      toStatus: newStatus,
      notes: `Rule-based analysis complete — ${fallback.agent3.overall_verdict} (confidence: ${fallback.agent3.confidence_score}%)`,
    });
