...
export async function registerRoutes(
  ...
  app.post('/api/admin/programs/:programId/extract-rules', authenticateUser, requireAdmin, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const pSystemPrompt = `You are an expert at analyzing loan credit policy documents and extracting ALL specific, actionable review rules with exhaustive, zero-miss accuracy.
...
- clarificationNeeded: true if the rule uses subjective language, has ambiguous terms, or could be interpreted multiple ways; false otherwise`;
  ...
  app.post('/api/admin/credit-policies/extract-rules', authenticateUser, requireAdmin, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const systemPrompt = `You are an expert at analyzing loan credit policy documents and extracting ALL specific, actionable review rules with exhaustive, zero-miss accuracy.
